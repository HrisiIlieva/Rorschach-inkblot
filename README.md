...
Each inkblot is modeled as a subset of the plane B \subset \mathbb{R}^2. The project analyzes the axial symmetry created by folding the paper, where (x,y) \in B \implies (-x,y) \in B. 
...
•	Complexity Range: Research indicates that inkblots have fractal properties with typical values falling between 1.1 < D < 1.3 or up to 1.6.
•	Calculation: The dimension is determined by D = \lim_{\epsilon \to 0} \frac{\log N(\epsilon)}{\log(1/\epsilon)}, where N(\epsilon) is the number of squares of size \epsilon needed to cover the blot.
...
•	Bayesian Perception: We model the probability P(s_i \mid B) of an observer seeing a specific object s_i based on factors like symmetry, contrast, and experience.
...
•	Information Entropy: Visual complexity is measured via Shannon entropy H = -\sum p_i \log p_i. Design aims for moderate entropy to avoid being too obvious or looking like noise.
## 💻 Simulation (Python)
